...
- **Native Multi-Runner CI/CD Pipeline**: GitHub Actions matrix workflow (`.github/workflows/desktop-release.yml`) building native standalone packages on native `ubuntu-latest` and `windows-latest` runners with SHA-256 checksums and automated release notes generation.
- **Release Automation Suite**: Added `scripts/release.sh` and `scripts/extract_changelog.py` for controlled, non-blind tag verification and release publishing.
