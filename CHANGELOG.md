...
- **Modular 4-Provider Speedtest Architecture (`desktop/core/speedtest/providers/`)**: Added dedicated, isolated adapters for Cloudflare, FAST.com, Ookla Speedtest, and nPerf with normalized `SpeedTestResult` data model, RFC 3550 jitter calculation, and vendor licensing discipline (marking UNAVAILABLE for uninstalled CLI or missing enterprise agents rather than scraping).
- **Sequential Bulk Benchmark & Cancellation Queue (`desktop/core/speedtest/runner.py`, `desktop/ui/dashboard/cockpit.py`)**: Implemented sequential bulk speed testing across all 4 providers without socket contention, complete with real-time UI progress updates (`Bulk Test X/4: Provider — STATUS`), safe multi-point cancellation, and individual provider fault tolerance.
- **Enhanced Speed Benchmark Cockpit Tab & Audit Trail (`desktop/ui/dashboard/cockpit.py`)**: Upgraded Speed Test tab with provider and target interface selectors, real-time download/upload/ping/jitter gauges, session results audit history table, and interactive detail inspection modals with full technical metadata.
- **Direct GitHub Release Navigation Links (`website/src/components/Landing/DownloadHub.tsx`, `HeroSection.tsx`, `App.tsx`)**: Made release badges, titles, and action buttons direct clickable links to the authoritative GitHub release page (`/releases/tag/v3.0.0-preview.23`), ensuring users can navigate directly to the published release and release notes.
- **Dynamic Release Discovery & Download Button Synchronization (`website/src/services/githubReleases.ts`, `website/src/App.tsx`, `website/src/components/Landing/HeroSection.tsx`, `website/src/components/Landing/DesktopShowcase.tsx`)**: Removed hardcoded preview version numbers across website fallbacks, dynamically resolving the latest GitHub release, and synchronized the exact same release tag across the navbar, hero section, desktop showcase, and download hub.
...
- **Dynamic Release Discovery with SWR Revalidation & Manual Refresh (`website/src/hooks/useLatestRelease.ts`, `website/src/services/githubReleases.ts`)**: Implemented Stale-While-Revalidate pattern that immediately revalidates with live GitHub Releases API on page load and updates the UI reactively to the latest release (`v3.0.0-preview.20`), eliminating stale browser cache locks and adding a manual Refresh button.
...
- **GitHub Releases API CORS Preflight Fix (`website/src/services/githubReleases.ts`)**: Removed forbidden `'Cache-Control'` request header from client-side GitHub API requests. GitHub's CORS preflight does not whitelist `Cache-Control` in `Access-Control-Allow-Headers`, which previously caused browsers to block live release discovery with `Cross-Origin Request Blocked`. Cache busting is now handled safely via URL query timestamp parameter.
- **Windows Teardown & Kernel BSOD Prevention**: Hardened `probe_socket_rtt` with `SO_REUSEADDR` and zero-linger `SO_LINGER(1, 0)` options to immediately discard sockets upon closure and abort with TCP RST, eliminating kernel `TIME_WAIT` socket accumulation in `tcpip.sys` during long-running sessions with OBS, vMix, and Zoom.
- **Ultra-Light Workload Detection & Process Handle Shielding**: Added 10-second TTL caching to `WorkloadWatcher` and protected `psutil.process_iter` with `NoSuchProcess`/`AccessDenied`/`ZombieProcess` shields, removing continuous `OpenProcess` scanning on every tick and preventing driver lock contention with vMix/OBS media capture drivers.
- **Graceful Multi-Stage Application Teardown & Route Metric Restoration**: Implemented 4-stage shutdown lifecycle in `AutoFailoverApp` and `CockpitDashboard`, cancelling scheduled Tkinter `after()` callbacks, cleanly stopping background orchestrator threads, cancelling in-flight speedtests, and restoring Windows network adapter metrics to `AutomaticMetric Enabled` upon exit.
